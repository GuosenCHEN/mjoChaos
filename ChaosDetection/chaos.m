...
% gaussian_transform %
    ...
    % 1 - normality-transform using Box-Cox 
...
if h==1
    ...
    if size(surr_y2,1)>size(surr_y2,2)
        surr_y2=surr_y2';
    end

    perm_n = 7; %default 7 for MJO test
    ...
    if strcmp(surrogate_algorithm, 'aaft_cpp')
        ...
        sig=params.cutsig;
        for i = 1:num_surr
            %surr_h1(i) = petropy(surr(i,:),8,1); %default
            surr_h1(i) = petropy(surr(i,:),perm_n,1);
        end 
        %perm_h1 = petropy(sig,8,1); %default
        perm_h1 = petropy(sig,perm_n,1);
        ...
        try
            %[surr, params] = surrogate(surr_y, num_surr, 'CPP', 1, 1); %default
            [surr, params] = surrogate(surr_y2, num_surr, 'CPP', 1, 1);
        catch
            %[surr, params] = surrogate(zscore(surr_y), num_surr, 'CPP', 1, 1); %default
            [surr, params] = surrogate(zscore(surr_y2), num_surr, 'CPP', 1, 1);
        end
        sig=params.cutsig;
        for i = 1:num_surr
            x1 = surr(i,:);
            %surr_h2(i) = petropy(x1,8,1); %default
            surr_h2(i) = petropy(x1,perm_n,1);
        end
        %perm_h2 = petropy(sig,8,1); %default
        perm_h2 = petropy(sig,perm_n,1);
        ...
        if stochastic1
            tmp_output = 'linear stochastic'
        elseif stochastic2
            tmp_output = 'nonlinear stochastic'
        end
        ...
        sig=params.cutsig;
        for i = 1:1000
            %surr_h(i) = petropy(surr(i,:),8,1); %default
            surr_h(i) = petropy(surr(i,:),perm_n,1);
        end
        %perm_h = petropy(sig,8,1); %default
        perm_h = petropy(sig,perm_n,1);
    ...
    if stochastic
        ...
        %output.permutation_entropy = petropy(y,5,1); %default
        perm_n2 = 5;
        output.permutation_entropy = petropy(y,perm_n2,1);
