...
def delay_embedding(data, emb_dim, lag):
  """
...
def lyap_r2(data, orbit, emb_dim=10, lag=None, min_tsep=None, tau=1, min_neighbors=20,
           trajectory_len=20, fit="RANSAC", debug_plot=False, debug_data=False,
           plot_file=None, fit_offset=0):
  """
  Estimates the largest Lyapunov exponent using the algorithm of Rosenstein
  ...
  if len(ks) < 1:
    ...
    # normal line fitting
    #poly = poly_fit(ks[fit_offset:], div_traj[fit_offset:], 1, fit=fit) #int(len(csums)/2):
    poly = poly_fit(ks[int(len(div_traj)/2):], div_traj[int(len(div_traj)/2):], 1, fit=fit) 
  if debug_plot:
    plot_reg(
      ks[fit_offset:], div_traj[fit_offset:],
      poly, "k", "log(d(k))", fname=plot_file)
...
def corr_dim(data, emb_dim, time_delay, rvals=None, dist=rowwise_euclidean,
  ...
  n = len(data)
  orbit = delay_embedding(data, emb_dim, time_delay)
  ...
  if len(csums) == 0:
    ...
    poly = poly_fit(np.log(rvals[int(len(csums)/2):]), np.log(csums[int(len(csums)/2):]), 1, fit=fit)
  if debug_plot:
...
def corr_dim2(data, orbit, rvals=None, dist=rowwise_euclidean,
             fit="RANSAC", debug_plot=False, debug_data=False, plot_file=None):
  """
  Calculates the correlation dimension with the Grassberger-Procaccia algorithm
  ...
  # TODO do this for multiple values of emb_dim?
  if rvals is None:
    sd = np.std(orbit, axis=0, ddof=1)[0]
    #sd = np.std(orbit, ddof=1)
    rvals = logarithmic_r(0.1 * sd, 0.5 * sd, 1.03)
  n = len(data)
  #orbit = delay_embedding(data, emb_dim, time_delay)
  dists = np.array([dist(orbit, orbit[i]) for i in range(len(orbit))])
